...
(() => {
  ...
  const STORE_KEY = 'my-farm-miniapp-v7-multifile';
  ...
  const configs = {
    coop: { count:'chickens', level:'coopLevel', capacity:'coopCapacity', product:'eggs', productName:'яйца', unit:'курица', buyCost:100, addCapacity:4, maxLevel:4 },
    pigsty: { count:'pigs', level:'pigstyLevel', capacity:'pigstyCapacity', product:'meat', productName:'мясо', unit:'свинья', buyCost:300, addCapacity:1, maxLevel:4 },
    sheepfold: { count:'sheep', level:'sheepfoldLevel', capacity:'sheepfoldCapacity', product:'wool', productName:'шерсть', unit:'овца', buyCost:250, addCapacity:2, maxLevel:4 },
    cowbarn: { count:'cows', level:'cowbarnLevel', capacity:'cowbarnCapacity', product:'milk', productName:'молоко', unit:'корова', buyCost:500, addCapacity:1, maxLevel:4 }
  };
  ...
  function buyAnimal(s){
    ...
    state[c.count] += 1;
    save(); render();
    toast(c.unit[0].toUpperCase() + c.unit.slice(1) + ' куплена');
  }
  ...
  const actions = {
    ...
    collectMilk: () => { collectAnimal('cowbarn'); closeModal(); },
    collectWool: () => { collectAnimal('sheepfold'); closeModal(); },
    collectMeat: () => { collectAnimal('pigsty'); closeModal(); },
  ...
  function openWallet(){
    modal('Баланс', resourcesHtml() + `<div class="note">Баланс сохраняется в localStorage. В Telegram Mini App экран не должен сворачиваться свайпом вниз.</div>`);
  }
  ...
  function openMenu(){
    modal('Меню', resourcesHtml() + `
      ...
      <div class="note">Версия v7: много файлов + папка assets.</div>`);
  }
  ...
  function openMail(){ modal('Почта', resourcesHtml() + `<div class="card">Письмо от мэра: “Ферма открыта для публичного запуска!”</div>`); }
  ...
  // screen-specific modal shortcut for screens that are also illustrated
  document.addEventListener('click', (e) => {
    // no-op here, hits handle it
  });

  // When entering these screens, tapping the big central content opens the matching modal too.
  const originalSetScreen = setScreen;
  window.openScreenModal = function(){
  ...
  // override render to add screen-specific content zones
  const baseRender = render;
  render = function(){
    hitLayer.innerHTML = '';
    hudLayer.innerHTML = '';
    const s = state.screen;
    if (s === 'home') { homeHits(); homeHud(); return; }
    if (['coop','pigsty','sheepfold','cowbarn'].includes(s)) { animalHits(s); animalHud(s); return; }
    hit(2,1.5,11,6, () => setScreen('home'), 'Назад');
    hit(5,12,90,66, () => window.openScreenModal(), label(s));
    hit(0,86,20,12, () => setScreen('home'), 'Ферма');
  ...
  // Prevent Safari / Telegram pull-down gestures as much as browser allows.
  let startY = 0;
